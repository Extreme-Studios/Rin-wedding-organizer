...
const storageKey = "rin-sahabat-ai-usage";
...
export default function ChatWidget() {
  ...
  const [messages, setMessages] = useState<ChatMessage[]>([
    { role: "assistant", text: "Halo, Kak! Aku Sahabat AI Rin. Mau tanya layanan, paket, atau butuh bantuan memahami typo chat?" },
  ]);
  ...
  async function sendMessage(event: FormEvent) {
    ...
    try {
      ...
      if (!response.ok) throw new Error(data.error || "Tidak dapat menghubungi Sahabat AI.");
      setRemaining(consumeUsage());
      setMessages((current) => [...current, { role: "assistant", text: data.reply }]);
    } catch (error) {
      const fallback = error instanceof Error ? error.message : "Sahabat AI sedang gangguan.";
  ...
  return (
    <div className="fixed bottom-6 right-6 z-[55] flex flex-col items-end gap-3">
      {open && (
        <section aria-label="Sahabat AI Rin" className="w-[min(23rem,calc(100vw-2.5rem))] overflow-hidden rounded-[1.6rem] border border-champagne bg-white shadow-lift">
          <header className="flex items-center justify-between bg-gradient-to-r from-[#4a3f33] to-cocoa px-5 py-4 text-white">
            <div className="flex items-center gap-3">
              ...
              <div><h2 className="font-serif text-xl">Sahabat AI Rin</h2><p className="text-xs text-cream/75">Tanya layanan dan paket</p></div>
            </div>
          ...
          <form onSubmit={sendMessage} className="border-t border-champagne/60 bg-white p-3">
            ...
            <div className="mt-2 flex items-center justify-between gap-3 text-[0.65rem] text-taupe"><span>AI dapat keliru. Konfirmasi detail ke tim Rin.</span><a href={waLink} target="_blank" rel="noreferrer" className="shrink-0 text-gold-dark hover:underline">WhatsApp</a></div>
          </form>
        </section>
      )}

      <button type="button" onClick={() => setOpen((value) => !value)} aria-expanded={open} aria-label="Buka Sahabat AI Rin" className="flex h-15 items-center gap-2 rounded-full bg-gradient-to-r from-[#4a3f33] to-cocoa px-5 text-sm font-medium text-white shadow-lift transition hover:-translate-y-0.5">
        {open ? <X size={20} /> : <MessageCircle size={20} />} <span>Sahabat AI</span>
      </button>
    </div>
  );
}
